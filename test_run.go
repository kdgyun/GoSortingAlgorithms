...
func run(origin, verify []int) {
	...
	t12, eq12, err12 := CallParallelLPQuickSort(origin, verify, "parallel left-pivot quick sort")
	...
	pf += fmt.Sprintf("| %35s | %20d ns | %15d ms | %10t |%s\n", "parallel left-pivot quick sort", t12, t12/int64(time.Millisecond), eq12, err12)
	pf += fmt.Sprintf("+%s+\n\n\n", strings.Repeat("-", 97))
